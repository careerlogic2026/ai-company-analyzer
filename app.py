...
EDITOR_PROMPTS = {
    "1. 業界構造とビジネスモデル": "1500文字以上で執筆。収益構造を重厚に解説。",
    "2. 事業分析（収益構造と最新動向）": "1500文字以上で執筆。最新の提携・リリース事例を全投入して進化を描け。",
    "3. 競合比較・SWOT分析": "1500文字以上で執筆。ボトルネックを深掘りせよ。",
    "4. 組織にいる人材と求める人物像": "1500文字以上で執筆。実在の社員名を組み込み、組織カルチャーを詳述せよ。",
    "5. キャリアパスと市場価値推論": "1500文字以上で執筆。具体的な市場価値を推論せよ。"
}
...
if st.button("🔍 1. 大容量・マルチドメイン検索を実行", type="primary"):
    ...
    with st.spinner(f"Web全域から{years_query}の記事を最大100件規模で抽出中..."):
        ...
        # ==========================================
        # 2. PR検索 (公式リリース ＋ 外部メディアの2系統検索)
        # ==========================================
        pr_id_match = re.search(r'company_id/(\d+)', pr_url) if pr_url else None
        pr_id = pr_id_match.group(1) if pr_id_match else ""
        
        # クエリA: 公式(PR TIMES)
        q_official = f"{short_company} プレスリリース {years_query}"
        res_official = client.search(query=q_official, include_domains=["prtimes.jp"], max_results=50).get("results", [])
        
        # クエリB: 外部メディア(日経, ITmedia, Bridge, NewsPicks等)
        q_media = f"{short_company} (提携 OR 取材 OR 特集 OR ニュース) {years_query}"
        res_media = client.search(query=q_media, include_domains=["nikkei.com", "xtrend.nikkei.com", "itmedia.co.jp", "bridge.jp.net", "newspicks.com", "forbesjapan.com"], max_results=50).get("results", [])
        
        combined_pr = res_official + res_media
        seen_pr = set()
        ...
        res_wantedly = client.search(query=q_wantedly, max_results=50).get("results", [])

        q_hr_media = f"{short_company} (インタビュー OR 社員) {years_query}"
        res_hr_media = client.search(query=q_hr_media, include_domains=["note.com", "talentbook.jp", "fastgrow.jp"], max_results=50).get("results", [])

        combined_hr = res_wantedly + res_hr_media
        seen_hr = set()
...
if st.session_state.search_done:
    st.subheader("📚 2. 大容量リサーチ結果（チェックを入れた記事を全精読します）")
    ...
    for i, cat in enumerate(st.session_state.search_results.keys()):
        with tabs[i]:
            st.success(f"合計 {len(st.session_state.search_results[cat])} 件の候補を捕捉")
...
if st.session_state.research_done:
    ...
    for cat_name, p_text, agent in [("IR・財務", PROMPT_FIN, "財務戦略"), ("PR・ニュース", PROMPT_PR, "事業開発"), ("ヒト・組織", PROMPT_HR, "組織文化")]:
        ...
        with st.status(f"🕵️‍♂️ {agent}担当が {len(urls)} 件のソースを読解中...") as status:
            combined = ""
            for url in urls:
                t = extract_text_from_url(url)
                res = model.generate_content(f"事実:{t}\n任務:{p_text}")
                combined += f"\n◆{url}\n{res.text}\n"
    ...
    with st.status("👑 編集長が文脈を繋ぎながら執筆中...") as status:
        for title, prompt in EDITOR_PROMPTS.items():
            ...
            res = model.generate_content(f"全事実:{all_facts}\n既出:{chain}\n章:{title}\n指示:{prompt}")
            final_text += f"## {title}\n\n{res.text}\n\n---\n\n"
            chain += f"[{title}]\n{res.text[:300]}...\n"
        status.update(label="🎉 完成！", state="complete")

    st.markdown(final_text)
    doc = Document(); doc.add_heading(f"{company} 分析レポート", 0)
    for p in final_text.split('\n'):
        if p.startswith('## '): doc.add_heading(p[3:], 1)
        elif p.strip() and p != "---": doc.add_paragraph(p)
    bio = BytesIO(); doc.save(bio)
    st.download_button("📄 Word保存", data=bio.getvalue(), file_name="AnalysisReport.docx", type="primary")
