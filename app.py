...
PROMPT_ED = """あなたは超一流の戦略コンサルタント兼キャリアアドバイザーです。
...
with st.expander("⚙️ 各エージェントへの指示（プロンプト）を編集する", expanded=False):
    prompt_fin = st.text_area("🕵️‍♂️ 財務・戦略担当へ（IR/PDFファクト抽出）", value=PROMPT_FIN, height=120)
    prompt_pr = st.text_area("🕵️‍♂️ 広報担当へ（ニュース記事ファクト抽出）", value=PROMPT_PR, height=120)
    prompt_hr = st.text_area("🕵️‍♂️ ヒト・組織担当へ（インタビュー記事ファクト抽出）", value=PROMPT_HR, height=120)
    prompt_ed = st.text_area("👑 統合エージェント（編集長）へ（最終レポート執筆）", value=PROMPT_ED, height=200)

if st.button("🔍 1. 対象メディアから個別記事をリストアップ", type="primary"):
    if not tavily_key: st.error("サイドバーにTavily APIキーを設定してください。"); st.stop()
    
    with st.spinner("Tavily AIを用いてドメイン内の個別記事を一本釣りしています..."):
        ...
        try:
            ...
            if hp_url:
                ...
                q_ir = f"site:{domain} 決算説明資料 OR 中期経営計画 OR 統合報告書 filetype:pdf"
                results["IR・財務"] = client.search(query=q_ir, max_results=5).get("results", [])
            
            # 2. PR TIMESの検索（ID一本釣り）
            if pr_url:
                pr_id = re.search(r'company_id/(\d+)', pr_url)
                q_pr = f"site:prtimes.jp/main/html/rd/p/ \"company_id/{pr_id.group(1)}\"" if pr_id else f"site:prtimes.jp/main/html/rd/p/ {company}"
                results["PR・ニュース"] = client.search(query=q_pr, max_results=5).get("results", [])
                
            # 3. Wantedlyの検索（スラッグ一本釣り）
            if rec_url:
                rec_slug = re.search(r'companies/([^/]+)', rec_url)
                q_hr = f"site:wantedly.com/companies/{rec_slug.group(1)}/post_articles OR site:wantedly.com/companies/{rec_slug.group(1)}/stories" if rec_slug else f"site:wantedly.com {company} インタビュー"
                results["ヒト・組織"] = client.search(query=q_hr, max_results=5).get("results", [])
...
# ==========================================
# UIセクション 2: ソース選択 (検索完了後に展開)
# ==========================================
if st.session_state.search_done:
    st.subheader("📚 2. 精読対象の選択")
    st.info("AIが特定した個別記事です。不要なものはチェックを外してください。")
